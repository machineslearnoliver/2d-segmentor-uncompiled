...
export class InputPanelComponent implements OnInit {
	...
	onNewMask() {
		...
		let imgFromCanvas = new Image();
		let canv = document.createElement('canvas');

		imgFromCanvas.onload = function() {
			canv.getContext('2d').drawImage(imgFromCanvas, 0, 0);
		};
		imgFromCanvas.src = this.canvasWhiteboard.generateCanvasDataUrl();
		console.log(imgFromCanvas);
		const w = 256;
		const h = 256;
